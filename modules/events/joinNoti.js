...
(async function () {
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body == `${prefix}request`) {
        adminBot.forEach(e => {
          api.sendMessage(`» ID: ${event.threadID}\n» ℹRequested Approval Successfully!☑`, e);
        })
        return api.sendMessage(`✔Sent a request to the admin bot(s)🔰!\n⚠️Please Wait Patiently For Your Request To Be Accepted⛔`, event.threadID);
      }
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`⚠️Your box/group has not been approved.\n⛔To submit a request for approval,\nuse: ${prefix}request\nBot Owner FB: fb.com/MdSagorMunshiXagor \n\nCHAND TRICKER`, event.threadID);
    };
    switch (event.type) {
      case "message":
      case "message_reply":
