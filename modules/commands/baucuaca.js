module.exports.config = {
	...
	hasPermssion: 0,
	credits: "Sunii",
	description: "Betting Games To Win More Money!",
	commandCategory: "games",
	usages: "[baucuaca] [amount] ",
	cooldowns: 3,
};
...
module.exports.run = async function({ api, event, args, Currencies }) {
			...
			if (coin < 20) return api.sendMessage(`Your bet is too small, the minimum is 20$!`, threadID, messageID);
			let number = [];
			...
			if (number[0] == number[1] && number[1] == number[2]) {
				...
				(win) ? api.sendMessage(`${slotItems[number[0]]} | ${slotItems[number[1]]} | ${slotItems[number[2]]}\nYou have won\n And Received ${coin} dollars Money Has Been Transferred To Your Account To Check Balance Use #coin.`, threadID, () => Currencies.increaseMoney(senderID, parseInt(coin)), messageID) : api.sendMessage(`${slotItems[number[0]]} | ${slotItems[number[1]]} | ${slotItems[number[2]]}\nYou have lost 😁\nThe amount you have bet belongs to the house`, threadID, () => Currencies.decreaseMoney(senderID, parseInt(coin)), messageID);
}
