...
"https://i.imgur.com/QYGOZuK.jpg",
  ...
  var money = data.money
      if(money < 350) api.sendMessage("You need 350 dollars to see photos!",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 350})
   var callback = () => api.sendMessage({body:`Dads really like it😼\n» Costs: 350$ Please wait for 5 secs for another.«`,attachment: fs.createReadStream(__dirname + "/cache/5.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/5.jpg"), event.messageID); 
