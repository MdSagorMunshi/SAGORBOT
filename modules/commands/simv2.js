...
module.exports.run = async function ({ api: b, event: a, args: c }) {
    ...
    switch (c[0]) {
        case "on":
            return global.simsimi.has(d) ? f("You have not turned off the Meta.") : (global.simsimi.set(d, e), f("Successfully enabled Meta."));
        case "off":
            return global.simsimi.has(d) ? (global.simsimi.delete(d), f("Successfully disabled Meta.")) : f("You have not enabled the Meta.");
        default:
